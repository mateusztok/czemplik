...
% --- Uruchomienie #4: Test zakłócenia dla ZN ---
Kp_ZN = Kp_ZN_styczna; Ki_ZN = Ki_ZN_styczna;
fprintf('Uruchamiam symulację #4 (Zakłócenie, nastawy ZN)...\n');
[t] = sim(model, czas);
t_URO_zak_ZN = t;
aTwew1_URO_zak_ZN = aTwew1;
aCVobiekt_URO_zak_ZN = CVobiekt;

% --- Uruchomienie #5: Test zakłócenia dla PID TUNE ---
Kp_ZN = Kp_pidtune; Ki_ZN = Ki_pidtune;
fprintf('Uruchamiam symulację #5 (Zakłócenie, nastawy PID TUNE)...\n');
[t] = sim(model, czas);
t_URO_zak_PID = t;
...
% 2.oba regulatory jako transmitancje
s = tf('s');
...
figure(9);
bodeplot(G_otwarty_ZN, 'r-', G_otwarty_PIDTUNE, 'b--');
title('Charakterystyki Bodego (Pętla otwarta, model URM)');
legend('ZN (styczna)', 'PID Tune', 'Location', 'southwest');
grid on;
...
fprintf('Zakończono. Wygenerowano poprawny wykres Bodego i tabelę wskaźników.\n');